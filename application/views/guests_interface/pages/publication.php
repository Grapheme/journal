...
<body>
	...
	<div class="wrapper">
		...
		<article>
			<header>
				<div class="all-pubs-link"><a href="<?=site_url('issue/'.$issue['id'].'-'.getTranslit($issue[$this->uri->language_string.'_title']));?>"><?=lang('all_publication_issue');?></a></div>
		...
		<?php endif;?>
			...
			<?php for($i=0;$i<count($keywords);$i++):?>
				<a href="<?=site_url('search?word='.md5(trim($keywords[$i])))?>"><?=$keywords[$i];?></a><?php if(isset($keywords[$i+1])):?>,<?php endif;?> 
			<?php endfor;?>
			</div>
			<?php endif;?>
